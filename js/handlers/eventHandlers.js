                    ...
                    const extractEventsPreview = async () => {
                        ...
                        for (let index = 0; index < eventContainers.length; index++) {
                            ...
                            const platforms = Array.from(platformIcons).map((icon, iconIndex) => {
                                ...
                                // Priority 1: Exact CSS class matches (most specific)
                                ...
                                // Priority 2: Check SVG content for unique patterns
                                const svgElement = icon.querySelector('svg');
                                if (svgElement) {
                                    const svgContent = svgElement.outerHTML;

                                    // LinkedIn specific patterns (most distinctive)
                                    if (svgContent.includes('M14.3921 4.77426') ||
                                        svgContent.includes('M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z')) {
                                        return 'LinkedIn';
                                    }

                                    // Instagram specific patterns
                                    if (svgContent.includes('M12.0833 1H3.91667C2.30608 1 1 2.30608 1 3.91667V12.0833C1 13.6939 2.30608 15 3.91667 15H12.0833') ||
                                        svgContent.includes('M14.9808 7.9989')) {
                                        return 'Instagram';
                                    }
                                ...
                                // Priority 3: Check aria-labels and attributes
                                const ariaLabel = icon.getAttribute('aria-label') || icon.getAttribute('alt') || icon.getAttribute('title') || '';
                                if (ariaLabel.toLowerCase().includes('linkedin')) return 'LinkedIn';
                                if (ariaLabel.toLowerCase().includes('facebook')) return 'Facebook';
                                if (ariaLabel.toLowerCase().includes('instagram')) return 'Instagram';
                                if (ariaLabel.toLowerCase().includes('youtube')) return 'YouTube';
                                if (ariaLabel.toLowerCase().includes('twitter') || ariaLabel.toLowerCase().includes('x')) return 'X';

                                // Priority 4: Conservative fallback checks (only if very specific)
